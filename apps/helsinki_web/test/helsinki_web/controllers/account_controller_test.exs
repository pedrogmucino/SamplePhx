defmodule AccountingSystemWeb.AccountControllerTest do
  ...
  @create_attrs %{apply_third_party_to: "no", apply_to: 42, character: false, code: "1-000-00-00000-000", group_code: 42, is_departamental: false, name: "Activos", parent_account: 42, payment_method: true, root_account: 1, status: "a", third_party_op: true, third_party_prosecutor: 42, type: "d", uuid_voucher: "some uuid_voucher", level: 0}
  ...
  def fixture(:accounts) do
    {:ok, account} = AccountHandler.create_account(@create_attrs)
    account
  end
  ...
  describe "update account" do
    ...
    test "redirects when data is valid", %{conn: conn, accounts: account} do
      conn = put(conn, Routes.account_path(conn, :update, account), account_schema: @update_attrs)
    ...
    test "renders errors when data is invalid", %{conn: conn, accounts: account} do
      conn = put(conn, Routes.account_path(conn, :update, account), account_schema: @invalid_attrs)
      assert html_response(conn, 200) =~ "Edit Account"
    end
  end
